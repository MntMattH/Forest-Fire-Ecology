...
RP_SWReGAP_Models[[1]] <- glm(Burned ~ SWReGAP_Class, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[1]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf
RP_SWReGAP_Models[[2]] <- glm(Burned ~ SWReGAP_Class * vpd_dsf * def_dsf * Tmax_dsf, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[2]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire
RP_SWReGAP_Models[[3]] <- glm(Burned ~  SWReGAP_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[3]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf
RP_SWReGAP_Models[[4]] <- glm(Burned ~ SWReGAP_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[4]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness
RP_SWReGAP_Models[[5]] <- glm(Burned ~ SWReGAP_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[5]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope
RP_SWReGAP_Models[[6]] <- glm(Burned ~ SWReGAP_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope, family = "binomial", data = CRP)
summary(RP_SWReGAP_Models[[6]])

#GLM with all LC * Aridity + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope
RP_SWReGAP_Models[[7]] <- glm(Burned ~ SWReGAP_Class * Aridity + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope, family = "binomial", data = CRP)
...
RP_SWReGAP_modeltable= aictab(cand.set = LF_Models, modnames = RP_SWReGAP_modelnames, second.ord = TRUE, nobs=NULL, sort=TRUE) #Make model selection table
...
#GLM with all LC 
RP_LF_Models[[1]] <- glm(Burned ~ LANDFIRE_Class, family = "binomial", data = CRP)
summary(RP_LF_Models[[1]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf
RP_LF_Models[[2]] <- glm(Burned ~ LANDFIRE_Class * vpd_dsf * def_dsf * Tmax_dsf, family = "binomial", data = CRP)
summary(RP_LF_Models[[2]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire
RP_LF_Models[[3]] <- glm(Burned ~  LANDFIRE_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire, family = "binomial", data = CRP)
summary(RP_LF_Models[[3]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf
RP_LF_Models[[4]] <- glm(Burned ~ LANDFIRE_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf, family = "binomial", data = CRP)
summary(RP_LF_Models[[4]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness
RP_LF_Models[[5]] <- glm(Burned ~ LANDFIRE_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness, family = "binomial", data = CRP)
summary(RP_LF_Models[[5]])

#GLM with all LC * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope
RP_LF_Models[[6]] <- glm(Burned ~ LANDFIRE_Class * vpd_dsf * def_dsf * Tmax_dsf + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope, family = "binomial", data = CRP)
summary(RP_LF_Models[[6]])

#GLM with all LC * Aridity + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope
RP_LF_Models[[7]] <- glm(Burned ~ LANDFIRE_Class * Aridity + Yrs_Since_Fire + PFNDVI_dsf + Ruggedness + Slope, family = "binomial", data = CRP)
summary(RP_LF_Models[[7]])

RP_LF_modelnames= c() #Model names 

RP_LF_modeltable= aictab(cand.set = LF_Models, modnames = RP_LF_modelnames, second.ord = TRUE, nobs=NULL, sort=TRUE) #Make model selection table
RP_LF_modeltable

################################################################################
#Below this point is a experimental way of sub-setting points in relation to FSC that is currently not being used. 
################################################################################
