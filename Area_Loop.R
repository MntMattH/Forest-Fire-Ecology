...
#Matt Harris 5/24/2023 (Updated 6/4/2023)
...
columns = c("Fire_ID", "DOB", "Area") 
Area_df<-data.frame(matrix(nrow = 657, ncol = length(columns)))
colnames(Area_df) = columns

Area_df$Fire_ID<-Fire_ID_lst
Area_df$DOB<-DOB_lst
Area_df$Area<-Area_lst

#Make AreaID metric by combining fire ID and DOB, in order to allow area to be matched to points from perimeter analysis. 

#This is currently not working due to the DOBs being in a list, but I'm unsure how to make these lists into string of individual DOBs to be combined with their Fire_ID. I'm stumped, but hope to come back to this with fresh eyes. 
#Area_df$AreaID<-paste(Area_df$Fire_ID, Area_df$DOB, sep = "_")
...
#Now that I have the area values, I need to get them to correspond to each DOB sampled by the matched points. To do this I will create a AreaID variable that is a combination of the Event_ID and DOB, allowing me to use a match function to add the area metric to matched points with a AreaID value. 
